...
for i in range(1, number_pi):
    ...
    for k in r:
        ...
        for h in range(cv):
            ...
            #SVM+Q model
            svmp_q = lr.SVMQplus()
            svmp_q.fit(X_trainr, X_trainp, y_train)
            test_svmp_q = svmp_q.predict(X_testr)
            svmplusq.append(accuracy_score(y_test, test_svmp_q))
...
gan_svmq = priv_gain(TACCsvmplusq, TACCsvmb, TACCsvmup)
...
data_svm = pd.DataFrame({#'nPI': range(1, number_pi),
                       ...
                       'TACCsvmplusq': TACCsvmplusq,
                       'TACCsvmup': TACCsvmup,
                       'Tstdsvmb': Tstdsvmb,
                       'Tstdsvmplus': Tstdsvmplus,
                       'Tstdsvmup': Tstdsvmup,                       
                       'Tstdsvmplusq': Tstdsvmplusq,
                       'gain_svm': gan_svm,
                       'gain_svmq':gan_svmq,
                       })
